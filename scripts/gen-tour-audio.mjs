#!/usr/bin/env node
/**
 * gen-tour-audio.mjs  —  Pre-generate TTS audio clips for all Scout tour narrations.
 *
 * Usage:    node scripts/gen-tour-audio.mjs
 * Output:   audio/tour/<hash>.mp3  (served at /audio/tour/<hash>.mp3)
 *
 * Reads every step narration from izzy-guide-config.js, computes a stable
 * 8-char hex hash per (voiceAgentId + '|' + narration), then fetches TTS
 * audio from the bill-talk ElevenLabs proxy and saves as a static .mp3.
 *
 * Idempotent: clips whose <hash>.mp3 already exists are skipped.
 * Regenerate: edit narrations, re-run, commit the new .mp3 files.
 */
...
const configSrc = readFileSync(join(ROOT, 'izzy-guide-config.js'), 'utf8');
const ctx = { window: {} };
runInNewContext(configSrc, ctx);
const cfg = ctx.window.SomaGuideConfig;
if (!cfg) {
  console.error('ERROR: Could not load SomaGuideConfig from izzy-guide-config.js');
  process.exit(1);
}
...
console.log(`Found ${narrations.length} unique narrations across all walkthroughs.\n`);
